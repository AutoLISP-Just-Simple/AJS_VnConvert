UVConverter 1.1.3b
Author: Pham Kim Long <longcz@yahoo.com>

UVConverter is a part of UniKey project. Visit UniKey website at: http://unikey.sourceforge.net 
This programme is released under the terms of the GNU General Public License (GPL). 
See the enclosed file gpl.txt

The programme uses the vnconv library (which is also a part of UniKey project).
If you get this source directory (uvconvert) from CVS, you must get 
the vnconv directory from CVS as well.

Compile instructions:

*Linux:
   - change to uvconvert directory
   - make

*Windows:
   - Start VC++.NET
   - Open solution file (uvcovert.sln) in uvconvert directory.
   - Build solution

After the compilation, you will get the generated executable file: 
uvconvert/uvconv (linux) or uvconvert/release/uvconv.exe (Windows).
...
Many thanks to VietLUGers (http://vietlug.sourceforge.net).
Special thanks to Nguyen Dai Quy <quy@nguyendai.org>.
